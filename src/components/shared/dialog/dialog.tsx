...
export function Dialog({
  wrapperClass = 'fixed w-full w-screen h-screen overflow-hidden top-0 left-0 z-100 flex flex-col items-center py-20',
  overlayClass = 'fixed w-full h-full top-0 left-auto pointer-events-none',
  dialogClass = 'relative shadow-md rounded-3xl m-auto',
  extraDialogClass = '',
  ...
  if (title && !header.length) {
    header = [
      <>
        <div className="flex flex-1 items-center" style={{ justifyContent: 'inherit' }}>
          {icon ? <i className="mr-2 text-lg text-accent">{icon}</i> : null}
          <span className="text font-semibold text-gray-700">{title}</span>
        </div>
        <button className="btn-default translate-x-4 transform" onClick={() => props.onClose()}>
          <i className="text-lg">
            <HiOutlineX />
          </i>
        </button>
